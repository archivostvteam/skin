# Embedded file name: /usr/lib/enigma2/python/Components/Converter/GlamourBase.py
...
class GlamourBase(Poll, Converter, object):
    ...
    def satname(self, tp):
        ...
        if orbe >= 179.7 and orbe <= 180.3:
            ...
            sat = 'Sim\xc3\xb3n Bol\xc3\xadvar'
    ...
    def orbital(self, tp):
        ...
        if orbp > 1800:
            orbp = str(float(3600 - orbp) / 10.0) + '\xc2\xb0W'
        else:
            orbp = str(float(orbp) / 10.0) + '\xc2\xb0E'
        return orbp
    ...
    def getText(self):
        ...
        if self.type == self.FREQINFO:
            ...
            elif 'DVB-S' in self.tunertype(tp):
